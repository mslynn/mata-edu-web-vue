<template>
  <div class="ai-image-history-page" :class="{ 'ai-image-history-page--embedded': embedded }">
...
const emit = defineEmits<{
  close: [];
  select: [item: ImageHistoryRecord];
}>();
...
const handleOpenImageDetail = (item: ImageHistoryRecord | null) => {
  ...
  if (!sessionId) return;
  if (props.embedded) {
    const matched = imageHistoryList.value.find((record) => record.sessionId === sessionId) || item;
    if (matched) {
      emit("select", matched);
    }
    return;
...
watch(
  ...
  (sessionId) => {
    ...
    if (!next) return;
    const matched = imageHistoryList.value.find((item) => item.sessionId === next);
    if (matched?.id) {
      selectedRecordId.value = matched.id;
      ensureSelectedRecordVisible();
    }
