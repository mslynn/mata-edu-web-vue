<template>
  <div
    ref="aiwendaPageRef"
    class="aiwenda-page"
    :class="{ 'aiwenda-page--embedded': embedded }"
    :style="pageAdaptiveStyle"
  >
    <div class="aiwenda-layout" :class="{ 'aiwenda-layout--embedded': embedded }">
      <TeacherSidebar v-if="!embedded" />

      <main class="aiwenda-main">
        <nav v-if="!embedded" class="aiwenda-breadcrumb" :aria-label="t('aiwenda.breadcrumbAria')">
...
const AIWENDA_MESSAGE_Z_INDEX = 2400;

const showSuccessMessage = (message: string) =>
  ElMessage({ message, type: "success", zIndex: AIWENDA_MESSAGE_Z_INDEX });

const showWarningMessage = (message: string) =>
  ElMessage({ message, type: "warning", zIndex: AIWENDA_MESSAGE_Z_INDEX });

const showErrorMessage = (message: string) =>
  ElMessage({ message, type: "error", zIndex: AIWENDA_MESSAGE_Z_INDEX });

const showConfirmDialog = (
  message: string,
  title: string,
  options: Record<string, unknown> = {},
) => ElMessageBox.confirm(message, title, { ...options, zIndex: AIWENDA_MESSAGE_Z_INDEX });

const showPromptDialog = (
  message: string,
  title: string,
  options: Record<string, unknown> = {},
) => ElMessageBox.prompt(message, title, { ...options, zIndex: AIWENDA_MESSAGE_Z_INDEX });
...
const handleDeleteRole = async (role: AiwendaRoleCard) => {
  ...
  try {
    ...
    showSuccessMessage(t("aiwenda.roleDeleted"));
    void loadOptRoleList();
  } catch (error: unknown) {
    showErrorMessage(error instanceof Error && error.message ? error.message : t("aiwenda.deleteRoleFailed"));
  }
};
...
const handleCreateRoleSubmit = async () => {
  ...
  if (!roleName) {
    showWarningMessage(t("aiwenda.inputRoleName"));
    return;
  }

  if (!prompt) {
    showWarningMessage(t("aiwenda.inputRolePrompt"));
    return;
  }
  ...
  try {
    if (isEditingRoleConfig.value) {
      ...
      showSuccessMessage(t("aiwenda.roleUpdated"));
    } else {
      await createOptQa({
        roleName,
        prompt,
      });
      showSuccessMessage(t("aiwenda.roleSaved"));
    }
    showRoleConfigDialog.value = false;
    roleConfigEditingRoleId.value = "";
    roleConfigTargetName.value = "";
    ...
    showErrorMessage(error instanceof Error && error.message ? error.message : t("aiwenda.saveRoleFailed"));
  } finally {
...
const handleSendMessage = async () => {
  ...
  if (!messageText && !currentFile?.ossId) {
    showWarningMessage(t("aiwenda.inputMessageOrUpload"));
    return;
  }
  ...
  try {
    ...
    if (resolvedSessionId) {
      ...
      emit("chat-session-ready", { sessionId: resolvedSessionId });
    }
...
const copyMessageContent = async (content: string) => {
  try {
    ...
    showSuccessMessage(t("aiwenda.copied"));
  } catch {
    showErrorMessage(t("aiwenda.copyFailed"));
  }
};
...
const handleHistoryClick = async (card: HistoryCard) => {
  ...
  try {
    ...
    emit("chat-session-ready", { sessionId: card.id });
    ...
    showErrorMessage(error?.message || t("aiwenda.loadSessionFailed"));
  }
};
...
const handleHistoryRename = async (card: HistoryCard) => {
  ...
  let newTitle = "";
  try {
    const { value } = await showPromptDialog(t("aiwenda.renamePrompt"), t("aiwenda.renameTitle"), {
  ...
  try {
    ...
    showSuccessMessage(t("aiwenda.renameSuccess"));
  } catch (error: any) {
    showErrorMessage(error?.message || t("aiwenda.renameFailed"));
  }
};
...
const handleHistoryPin = async (card: HistoryCard) => {
  ...
  try {
    ...
    await loadHistorySessions();
    showSuccessMessage(card.pinned ? t("aiwenda.unpinSuccess") : t("aiwenda.pinSuccess"));
  } catch (error: any) {
    showErrorMessage(error?.message || t("aiwenda.operationFailed"));
  }
};
...
const handleHistoryDelete = async (card: HistoryCard) => {
  hoverMenuId.value = "";
  try {
    await showConfirmDialog(t("aiwenda.deleteHistoryMessage"), t("aiwenda.tip"), {
  ...
  try {
    ...
    await loadHistorySessions();
    showSuccessMessage(t("aiwenda.deleteSuccess"));
  } catch (error: any) {
    showErrorMessage(error?.message || t("aiwenda.deleteFailed"));
  }
};
...
<style scoped src="../../pages/system/opt/aiwenda/aiwenda.scoped.css"></style>
<style src="../../pages/system/opt/aiwenda/aiwenda.global.css"></style>
<style scoped>
.aiwenda-page--embedded {
  min-width: 0;
  height: 100%;
  min-height: 0;
  overflow: hidden;
...
.aiwenda-page--embedded .aiwenda-main {
  min-height: 0;
  height: 100%;
  padding: 20px 20px 16px;
}

.aiwenda-page--embedded .aiwenda-content-grid {
  height: 100%;
  min-height: 0;
}

.aiwenda-page--embedded .aiwenda-workspace,
.aiwenda-page--embedded .aiwenda-history {
  min-height: 0;
}

.aiwenda-page--embedded .aiwenda-chat-list {
  min-height: 180px;
}

.aiwenda-page--embedded .aiwenda-composer {
  flex-shrink: 0;
}
</style>
