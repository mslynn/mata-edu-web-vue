...
// 本地默认图标（打包后为绝对可访问路径）
const homeIcon = new URL("../../assets/newimages/home.svg", import.meta.url)
  .href;
const classIcon = new URL("../../assets/newimages/class.svg", import.meta.url)
  .href;
const courseIcon = new URL("../../assets/newimages/cursor.svg", import.meta.url)
  .href;
const toolsIcon = new URL("../../assets/newimages/tool.svg", import.meta.url)
  .href;
const aiIcon = new URL("../../assets/newimages/ai.svg", import.meta.url).href;
const teacherIcon = new URL(
  "../../assets/newimages/student.svg",
  import.meta.url,
).href;
const studyIcon = new URL("../../assets/newimages/study.svg", import.meta.url)
  .href;
const eventIcon = new URL("../../assets/newimages/saishi.svg", import.meta.url)
  .href;
...
export const useTeacherNav = () => {
  ...
  const initializedState = useState<boolean>(
    "teacher-nav-initialized",
    () => false,
  );
  ...
  const normalizeSelectedIcon = (
    ...
    if (activeMenu && localSelectedIcons[activeMenu])
      return localSelectedIcons[activeMenu];
    // 关键词兜底
    const key = label || "";
    const hit = (kw: string) => key.includes(kw);
    if (hit("学情") || hit("学习") || hit("学情中心"))
      return localSelectedIcons["/study"] || fallback || "";
    if (hit("课程")) return localSelectedIcons["/course"] || fallback || "";
    if (hit("工具")) return localSelectedIcons["/tools"] || fallback || "";
    if (hit("AI")) return localSelectedIcons["/opt"] || fallback || "";
    if (hit("成长") || hit("教师"))
      return localSelectedIcons["/growth"] || fallback || "";
    if (hit("赛事") || hit("比赛"))
      return localSelectedIcons["/event"] || fallback || "";
    if (hit("账号") || hit("班级"))
      return localSelectedIcons["/system/class"] || fallback || "";
    return fallback || "";
  };
  ...
  const mapMenus = (data: any[]): TeacherMenuItem[] => {
    ...
    const mapped = data
      .map((item: any) => {
        ...
        const rawIconSelected =
          child.meta?.iconSelected || child.meta?.activeIcon;
        const icon = normalizeIcon(
          rawIcon,
          path,
      ...
      .filter(Boolean)
      .map((item) =>
        attachLocalIcons(item as TeacherMenuItem),
      ) as TeacherMenuItem[];
